...
void GeneratorServerSource::generate(std::ostream& stream)
{
    ...
    if (not mSettings.serverHeader().empty())
    {
        include = mSettings.serverHeader();
    }
...

    {
        ...
        stream << "extern " << mSettings.returnType() << " " << mSettings.callRpcName() << "(" << join(rpcCallParameters, ", ") << ");" << std::endl;
        stream << std::endl;
    }
    ...
    for (const Data& data : mSettings.data())
    {
        ...
        else if (data.type == DATA_METHOD)
        {
            ...
            if (not data.inOut.type.empty())
            {
                ...
                {
                    rpcItem.push_back(data.inOut.size);
                }
                else
                {
...

    {
        ...
        stream << "    res = " << mSettings.callRpcName() <<"(" << join(rpcCallArguments, ", ") << ");" << std::endl;
        stream << std::endl;
